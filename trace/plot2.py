...
def plot_metrics(
    ...
    for col in current_jobs_per_site.columns:
        axes[3].plot(current_jobs_per_site.index, current_jobs_per_site[col], label=col, linewidth=1.2)
    axes[3].axhline(150, color="black", linestyle="--", linewidth=1, label="Site max (150)")
    axes[3].set_ylabel("Current jobs/site")
    axes[3].set_title("Current existing jobs per site over time (logs-reconstructed, non-stacked)")
...
def main():
    ...
    plot_metrics(arrivals, runtime_sum, waiting_jobs, current_jobs_per_site)
